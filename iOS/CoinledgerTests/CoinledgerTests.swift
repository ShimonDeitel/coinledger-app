...
final class CoinledgerTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(CoinEntry(title: "Test", detail1: "A", detail2: "B", note: "", date: Date()))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(CoinEntry(title: "ToDelete", detail1: "A", detail2: "B", note: "", date: Date()))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        while store.entries.count < Store.freeLimit {
            store.add(CoinEntry(title: "Filler", detail1: "A", detail2: "B", note: "", date: Date()))
        }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(CoinEntry(title: "Original", detail1: "A", detail2: "B", note: "", date: Date()))
